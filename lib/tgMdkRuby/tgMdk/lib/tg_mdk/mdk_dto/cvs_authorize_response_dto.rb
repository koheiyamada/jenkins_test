...
module Veritrans
  module Tercerog
    module Mdk
      ...
      # 
      class CvsAuthorizeResponseDto < ::ResponseBaseDto
        ...
        # - ローソン：　URLなし
        # - その他：　URLあり
        # - 戻り値の「https://～.info/JLP/JLPcon」の部分を
        # - 「https://w2.kessai.info/JLM/JLMcon」に置換えてご利用ください。
        # - 例）
        # - https://w2.kessai.info/JLM/JLMcon?code=xxx～&rkbn=1
        # - ※PCブラウザーからの印刷用ですので、携帯では正しく表示できません。
        # - ※ウェルネットについては携帯での表示・支払に対応しております。
        # - ※テスト時は上記URLではなく、テスト用URLが返戻されます（アクセスはできません）。
        # @return:: 払込票URL 
        #
        def haraikomi_url 
            @haraikomi_url
        ...
        #
        # ===払込票URLを設定する 
        # @param :: haraikomiUrl 払込票URL 
        #
        def haraikomi_url=(haraikomiUrl) 
            @haraikomi_url = haraikomiUrl
        end
